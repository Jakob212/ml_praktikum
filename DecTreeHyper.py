...
for dataset in datasets:
    ...
    # Hyperparameter-Gitter für DecisionTreeClassifier
    param_grid = {
    ...
    for i in range(10):
        ...
        X_shuffled = X.sample(frac=1, random_state=i)
        ...
        # Ergebnisse speichern (beste Accuracy und zugehörige Parameter)
        accuracies.append(grid.best_score_)
        best_params_list.append(grid.best_params_)

    # Durchschnittliche Ergebnisse (Accuracy) + Standardabweichung
    mean_accuracy = np.mean(accuracies)
    std_accuracy = np.std(accuracies)

    # Besten Durchlauf finden (höchste Accuracy)
    best_run_idx = np.argmax(accuracies)
    best_run_accuracy = accuracies[best_run_idx]
    best_run_params = best_params_list[best_run_idx]

    print(f"Durchschnittliche Genauigkeit von {dataset}: {mean_accuracy:.4f} (Std: {std_accuracy:.4f})")
    print(f"Bester Durchlauf mit Accuracy: {best_run_accuracy:.4f}")
    print(f"Hyperparameter: {best_run_params}")

    # Häufigste Parameterkombination finden
    # 1) Mach aus jedem dict ein sortiertes Tupel, damit es hashbar wird:
    param_tuples = [tuple(sorted(params.items())) for params in best_params_list]
    # 2) Zähle Vorkommen
    counter = Counter(param_tuples)
    most_common_tup, freq = counter.most_common(1)[0]  # häufigstes Element und Häufigkeit
    # Zurückwandeln in dict (optional):
    most_common_params = dict(most_common_tup)

    print(f"Häufigste Parameterkombination: {most_common_params} (in {freq} von 10 Durchläufen)\n")
